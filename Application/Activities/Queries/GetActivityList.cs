                ...
                IRequestHandler<Query, Result<PageList<ActivityDto, DateTime?>>>
    {
        public async Task<Result<PageList<ActivityDto, DateTime?>>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrEmpty(request.Params.Filter))
            {
                ...
                {
                    "isGoing" => userId == null
                        ? query.Where(a => false) // no activities if not authenticated and filter "isGoing"
                        : query.Where(x => x.Attendees.Any(a => a.UserId == userId)),
                    "isHost" => userId == null
                        ? query.Where(a => false) // no activities if not authenticated and filter "isHost"
                        : query.Where(x => x.Attendees.Any(a => a.IsHost && a.UserId == userId)),
                    _ => query
                };
            }
